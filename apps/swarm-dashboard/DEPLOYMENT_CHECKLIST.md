...
**On Staging Server**:
```bash
ssh ubuntu@staging.atlas-sphere.com
...
```bash
ssh ubuntu@prod.atlas-sphere.com
...
| `STAGING_HOST` | `staging.atlas-sphere.com` (or IP) |
| `STAGING_USER` | `ubuntu` |
| `STAGING_KEY` | *(Contents of ~/.ssh/dashboard-staging)* |
| `PROD_HOST` | `prod.atlas-sphere.com` (or IP) |
| `PROD_USER` | `ubuntu` |
...
4. Monitor in:
   ...
   - Staging URL: https://staging.atlas-sphere.com
...
- [ ] Dashboard accessible: `curl https://dashboard.atlas-sphere.com`
- [ ] Returns HTTP 200
