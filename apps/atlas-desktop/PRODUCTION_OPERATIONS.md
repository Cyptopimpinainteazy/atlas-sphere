# Production Operations Guide - Atlas Desktop
...
```bash
# 1. Check error logs
tail -f ~/.atlas-desktop/logs/main.log
...
grep -i "error" ~/.atlas-desktop/logs/main.log | wc -l

# 2. Verify no crash dumps
ls -la ~/.atlas-desktop/logs/crash/ 2>/dev/null || echo "None"
...
4. Force quit and restart: `pkill -f atlas-desktop`
...
```bash
# Check error rate
tail -100 ~/.atlas-desktop/logs/main.log | grep -i error | wc -l
...
### Logs Location
```bash
~/.atlas-desktop/logs/main.log
~/.atlas-desktop/logs/crash/
~/.atlas-desktop/config/app-config.json
```
...
```bash
# Run tests
cd apps/atlas-desktop && npm run test
