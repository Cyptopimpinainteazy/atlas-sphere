/**
 * Substrate Client for Atlas Desktop (ported from explorer)
 *
 * Singleton connection to the Atlas Sphere Substrate node
...
async function createConnection(endpoint: string): Promise<ApiPromise> {
  ...
  const api = await ApiPromise.create({ provider, types: ATLAS_TYPES, rpc: ATLAS_RPC });
  await api.isReady;
