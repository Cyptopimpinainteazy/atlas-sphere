 ...
 * 
 * Handles launching and managing third-party apps from the Atlas Desktop.
...
export class AppLauncherService {
  ...
  async launchApp(app: AppStoreApp): Promise<LaunchResult> {
    ...
    try {
      ...
      if (!appDirExists) {
        return {
          success: false,
          message: `App directory not found: ${appPath}. Run 'apps/atlas-desktop/app-store/setup-treasury-integration.sh' to install/copy apps into Tauri appDataDir.`,
        };
      }
  ...
  async installApp(app: AppStoreApp): Promise<LaunchResult> {
    try {
      ...
      const repoRoot = homeEnv ? `${homeEnv}/Desktop/atlas-sphere-master` : '/workspace';
      const setupScript = `${repoRoot}/apps/atlas-desktop/app-store/setup-treasury-integration.sh`;
  ...
  private logAppLaunch(app: AppStoreApp): void {
    ...
    try {
      const logs = JSON.parse(localStorage.getItem("atlas-desktop:app-launches") || "[]");
      logs.push(logEntry);
      if (logs.length > 100) logs.splice(0, logs.length - 100);
      localStorage.setItem("atlas-desktop:app-launches", JSON.stringify(logs));
    } catch (error) {
