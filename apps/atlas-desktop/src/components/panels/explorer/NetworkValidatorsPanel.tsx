...
const validators = [
  { name: 'Atlas Foundation', address: '5GrwvaEF5zXb26Fz9rcQpDWS57C', staked: 8200000, commission: 3, uptime: 99.98, status: 'active' },
...
const rpcProviders = [
  { name: 'Atlas Public RPC', endpoint: 'https://rpc.atlas.network', latency: 45, requestsDay: 2400000, status: 'healthy' },
  { name: 'Alchemy Atlas', endpoint: 'https://atlas.g.alchemy.com/v2/...', latency: 32, requestsDay: 8500000, status: 'healthy' },
  { name: 'Infura Atlas', endpoint: 'https://atlas.infura.io/v3/...', latency: 38, requestsDay: 6200000, status: 'healthy' },
  { name: 'QuickNode', endpoint: 'https://atlas.quiknode.pro/...', latency: 28, requestsDay: 4100000, status: 'healthy' },
  { name: 'Ankr', endpoint: 'https://rpc.ankr.com/atlas', latency: 55, requestsDay: 1800000, status: 'degraded' },
  { name: 'BlastAPI', endpoint: 'https://atlas.blastapi.io/...', latency: 42, requestsDay: 950000, status: 'healthy' },
];
          ...
          <div>
            <div className="grid grid-cols-3 gap-3 mb-5">
              {[{ label: 'Active Validators', value: activeCount, sub: `${validators.length} total` },
                { label: 'Total Staked', value: `$${(totalStaked / 1e6).toFixed(1)}M`, sub: 'ATLAS' },
              ...
              {rampProviders.filter(p => p.supports.includes(rampType)).map((p, i) => (
                <div key={i} className="p-4 bg-[#111118] border border-[#1a1a1a] rounded-lg">
                  ...
                  <button className="w-full mt-3 py-1.5 bg-[#ff6b35]/10 text-[#ff6b35] text-xs rounded hover:bg-[#ff6b35]/20 transition-colors">
                    {rampType === 'on' ? 'Buy ATLAS' : 'Sell ATLAS'}
                  </button>
                </div>
              ))}
            </div>
