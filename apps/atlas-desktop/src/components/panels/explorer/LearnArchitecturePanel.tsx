...
const concepts = [
  { icon: <Layers size={18} />, title: 'Dual-VM Architecture', description: 'Atlas Sphere executes both EVM and SVM runtimes within a single Substrate-based blockchain. The Atlas Kernel manages VM scheduling, state isolation, and cross-VM message passing. Developers deploy Solidity or Rust contracts and they coexist on the same chain.' },
...
const utilities = [
  { title: 'Gas Fees', description: 'ATLAS is used to pay transaction fees on both EVM and SVM execution.' },
  { title: 'Staking', description: 'Validators stake ATLAS to participate in block production and earn rewards.' },
  { title: 'Governance', description: 'STAR tokens are used for on-chain governance voting and proposal submission.' },
  { title: 'Swarm Rewards', description: 'GPU swarm node operators earn ATLAS for completing computation jobs.' },
];

const tutorials = [
  { title: 'Your First Atlas dApp', difficulty: 'Beginner', time: '30 min', description: 'Set up a local dev node, deploy a simple EVM contract, and interact with it through the Atlas SDK.' },
  ...
  { title: 'Custom Pallet Development', difficulty: 'Advanced', time: '90 min', description: 'Build a custom Substrate pallet that extends the Atlas runtime with new functionality.' },
  { title: 'Zero-Knowledge Proofs on Atlas', difficulty: 'Advanced', time: '120 min', description: 'Implement and verify ZK proofs using the Atlas verifier pallet and GPU swarm for proof generation.' },
];
        ...
        {activeTab === 'architecture' && (
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Atlas Sphere unifies two virtual machines under a single consensus layer. The diagram below shows the primary data flow from user transactions through the Atlas Kernel to final state commitment.
            </p>
...
└──────────┬──────────────────┬───────────────────────────┘
     ...
     └──────────────────────────┘`}
            ...
            <div className="space-y-4">
              {[
                { title: 'Atlas Kernel', desc: 'The core orchestration layer that routes transactions to the appropriate VM, manages the Comit Engine for cross-VM calls, and coordinates block production.' },
        ...
        {activeTab === 'concepts' && (
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Understanding these five foundational concepts will give you a complete picture of how Atlas Sphere works under the hood.
            </p>
            <div className="space-y-4">
              {concepts.map((c, i) => (
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="p-4 bg-[#111118] border border-[#1a1a1a] rounded-lg">
                ...
                <p className="text-lg font-bold text-white">ATLAS</p>
              ...
              <div className="p-4 bg-[#111118] border border-[#1a1a1a] rounded-lg col-span-2">
                ...
                <p className="text-lg font-bold text-white">1,000,000,000 ATLAS</p>
            ...
            <div className="space-y-2 mb-6">
              {distribution.map((d, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-500 w-24 text-right">{(d.pct * 10_000_000).toLocaleString()} ATLAS</span>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'tutorials' && (
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-sm text-gray-400 mb-6">Step-by-step guides to help you build on Atlas Sphere, from beginner to advanced.</p>
