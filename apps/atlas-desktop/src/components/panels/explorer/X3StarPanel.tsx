...
/* ------------------------------------------------------------------ */
const BOOT_LINES = [
  "x3Star Terminal v3.1.0 — Atlas Sphere Kernel",
  "Loading runtime modules ██████████████████ OK",
  "Connecting to X3 Atlas Chain (peers: 42) … OK",
...
export default function X3StarPanel() {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      setInput("");
      const next = [...lines, { type: "input" as const, text: `atlas@x3star:~$ ${input}` }];
      if (cmd === "clear") {
        setLines([]);
        return;
      }
  ...
  return (
    <div className="overflow-hidden h-full bg-black text-green-400 font-mono flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-[#0a0a0a] border-b border-green-900/40 text-xs">
        ...
        <span className="text-green-800">atlas-sphere</span>
      </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        >
          ...
          {bootDone && (
            ...
            >
              <span className="text-green-400 mr-2 text-xs">atlas@x3star:~$</span>
              <input
                ref={inputRef}
                value={input}
