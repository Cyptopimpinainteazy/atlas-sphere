...
const RpcStatsPanel: React.FC = () => {
  ...
  const plans = [
    ...
    { name: 'Atlas Sphere', cost: 0, color: '#10b981', desc: 'Free forever 🎯' },
  ];
  ...
  return (
    <div style={s.root}>
      ...
      <div style={{ padding: '8px 16px', borderTop: '1px solid #1a1f2e', textAlign: 'center', color: '#444', fontSize: '0.6rem', flexShrink: 0 }}>
        Atlas Sphere RPC Pool — {data.healthy_endpoints} healthy endpoints across {data.chains_covered} chains — refreshes every 10s
      </div>
    </div>
  );
};
