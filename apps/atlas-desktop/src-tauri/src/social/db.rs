...
impl SocialDb {
    pub fn new(app_dir: PathBuf) -> SqlResult<Self> {
        ...
        let db_path = app_dir.join("atlas_social.db");
    ...
    fn run_migrations(&self) -> SqlResult<()> {
        ...
        if count == 0 {
            ...
            conn.execute_batch(&format!("
                ...
                VALUES
                    ('tc-001', 'ATLAS-TEAM-2026',   'Core Team',     'team',  0, 0, 'system', '{ts}', 1),
                    ('tc-002', 'ATLAS-ADMIN-KEY',    'Admin',         'admin', 0, 0, 'system', '{ts}', 1),
                    ('tc-003', 'ATLAS-DEV-ACCESS',   'Developer',     'team',  0, 0, 'system', '{ts}', 1),
                    ('tc-004', 'ATLAS-VIP-INVITE',   'VIP',           'vip',   50, 0, 'system', '{ts}', 1);
            ")).ok();
        }
