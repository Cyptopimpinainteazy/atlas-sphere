...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn complete_deregister(origin: OriginFor<T>) -> DispatchResult {
            ...
            let contributor = Contributors::<T>::get(contributor_id)
                .ok_or(Error::<T>::ContributorNotFound)?;
            ...
            let current_block = frame_system::Pallet::<T>::block_number();
            let cooldown_end = contributor.deregister_at.saturating_add(T::UnstakeCooldown::get());
            ensure!(current_block >= cooldown_end, Error::<T>::CooldownNotExpired);
        ...
        pub fn submit_task(
            ...
            let task_id: TaskId = BlakeTwo256::hash_of(&(
                &submitter,
                &workload_type,
                &payload_hash,
                &current_block,
        ...
        pub fn finalize_session(origin: OriginFor<T>, session_id: SessionId) -> DispatchResult {
            ...
            let session = JurySessions::<T>::get(session_id)
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_distribute_reward(
            ...
            let contributor_share: BalanceOf<T> = contributor_share_u128
                .try_into()
                .unwrap_or(Zero::zero());
