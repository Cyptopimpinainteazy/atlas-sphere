...
fn deregister_provider_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(DepinMarketplace::deregister_provider(RuntimeOrigin::signed(1)));
...
fn revenue_split_conservation() {
    new_test_ext().execute_with(|| {
        ...
        let job_id = DepinMarketplace::pending_orders()[0].job_id;
        assert_ok!(DepinMarketplace::accept_order(RuntimeOrigin::signed(1), job_id));
...
fn cancel_order_refunds_escrow() {
    new_test_ext().execute_with(|| {
        ...
        let job_id = DepinMarketplace::pending_orders()[0].job_id;
        assert_ok!(DepinMarketplace::cancel_order(RuntimeOrigin::signed(10), job_id));
