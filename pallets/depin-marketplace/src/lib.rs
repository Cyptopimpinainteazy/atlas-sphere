...
pub mod pallet {
        ...
        fn on_initialize(now: BlockNumberFor<T>) -> Weight {
            ...
            for (job_id, job) in expired_jobs {
                ...
                Self::deposit_event(Event::JobFailed {
                    ...
                    provider: job
                        .assigned_provider
                        .unwrap_or(T::AccountId::decode(&mut sp_runtime::traits::TrailingZeroInput::zeroes()).expect("infinite")),
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn accept_order(origin: OriginFor<T>, job_id: JobId) -> DispatchResult {
            ...
            let order = PendingOrders::<T>::try_mutate(|orders| -> Result<Order<T>, DispatchError> {
                let idx = orders
                    .iter()
            ...
            Providers::<T>::try_mutate(&who, |maybe_provider| -> DispatchResult {
                if let Some(provider) = maybe_provider.as_mut() {
                    provider.total_jobs_completed =
                        provider.total_jobs_completed.saturating_add(1);
        ...
        fn slash_provider(provider: T::AccountId, _job_id: &JobId) {
            if let Some(mut info) = Providers::<T>::get(&provider) {
                ...
                let (imbalance, _remaining) =
                    T::Currency::slash_reserved(&provider, slash_amount);
                T::BurnDestination::on_unbalanced(imbalance);

                info.stake = info.stake.saturating_sub(
                    slash_amount.saturated_into(),
                );
