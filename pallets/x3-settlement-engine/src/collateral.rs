...
impl<AccountId: Clone + PartialEq, Balance: Copy + PartialOrd + core::ops::Sub<Output = Balance> + Default>
    ...
    CollateralManager<AccountId, Balance> for InMemoryCollateral<AccountId, Balance>
{
    ...
    fn request_withdraw(&mut self, bond_id: H256) -> Result<(), CollateralError> {
        let b = self.bonds.iter_mut().find(|b| b.id == bond_id).ok_or(CollateralError::BondNotFound)?;
    ...
    fn finalize_withdraw(&mut self, bond_id: H256) -> Result<(), CollateralError> {
        let idx = self.bonds.iter().position(|b| b.id == bond_id).ok_or(CollateralError::BondNotFound)?;
...
mod tests {
    ...
    fn test_deposit_and_withdraw_flow() {
        ...
        let id = c.deposit_bond(1u64, b"USDC".to_vec(), 1_000u128, BondType::PerformanceBond).unwrap();
    ...
    fn test_slash_flow() {
        let mut c = InMemoryCollateral::<u64, u128>::new();
        let id = c.deposit_bond(2u64, b"USDC".to_vec(), 5_000u128, BondType::InitialMargin).unwrap();
