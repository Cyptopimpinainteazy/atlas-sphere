...
pub mod pallet {
    ...
    pub type CommitteePublicKey<T: Config> = StorageValue<_, BoundedVec<u8, ConstU32<64>>, OptionQuery>;
    ...
    impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
        fn build(&self) {
            ...
            assert_eq!(total, 10_000, "Private execution fee split must sum to 10_000 bps");
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_private_transaction(
            ...
            let premium = base_fee.saturating_mul(
                premium_bps.try_into().map_err(|_| Error::<T>::ArithmeticOverflow)?
            ) / 10_000u32.into();
        ...
        pub fn commit_encrypted_state_diff(
            ...
            let diff = EncryptedDiff {
                ...
                encrypted_state_changes: BoundedVec::try_from(encrypted_state_changes)
                    .map_err(|_| Error::<T>::PayloadTooLarge)?,
                commitment,
                zk_proof: zk_proof.map(|p| {
                    BoundedVec::try_from(p).unwrap_or_default()
                }),
                enclave_signature,
        ...
        pub fn set_committee_key(
            origin: OriginFor<T>,
            public_key: Vec<u8>,
        ) -> DispatchResult {
            T::AdminOrigin::ensure_origin(origin)?;

            let bounded = BoundedVec::try_from(public_key)
                .map_err(|_| Error::<T>::PayloadTooLarge)?;
