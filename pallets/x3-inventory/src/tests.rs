...
fn vault_id(seed: u8) -> VaultId { [seed; 32] }
fn lane_id(seed: u8) -> LaneId { [seed; 32] }
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            X3Inventory::create_vault(
                ...
                VaultType::Gas, OwnerType::Protocol, 1, 100,
                100, 500, 1_000, 2_000,
            ),
...
fn update_vault_bands_emits_event_and_persists() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::update_vault_bands(
            RuntimeOrigin::root(), id, 200, 600, 1_200, 3_000
        ));
...
fn update_vault_bands_re_evaluates_status_to_frozen() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::update_vault_bands(
            RuntimeOrigin::root(), id, 1_000, 2_000, 3_000, 5_000
        ));
...
fn register_lane_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        register_test_lane(id);
        System::assert_has_event(
            Event::LaneRegistered { lane_id: id, lane_class: LaneClass::A }.into(),
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            X3Inventory::register_lane(
                RuntimeOrigin::root(), id,
                1, 2, 100, 200, LaneClass::A, sources, 1_000_000, 500_000,
            ),
...
fn freeze_already_frozen_lane_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        register_test_lane(id);
        assert_ok!(X3Inventory::freeze_lane(
            RuntimeOrigin::root(), id, FreezeReason::OperatorManual
        ));
        assert_noop!(
...
fn unfreeze_lane_restores_active_and_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::unfreeze_lane(RuntimeOrigin::root(), id, evidence.clone()));

        let lane = Lanes::<Test>::get(id).unwrap();
        assert_eq!(lane.status, LaneStatus::Active);
        System::assert_has_event(
            Event::LaneUnfrozen { lane_id: id, operator_id: [0xbb; 32] }.into(),
        );
    });
}
...
fn freeze_nonexistent_lane_returns_error() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            X3Inventory::freeze_lane(
                RuntimeOrigin::root(), lane_id(99), FreezeReason::OperatorManual
            ),
...
fn reserve_inventory_moves_balance_from_available_to_reserved() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 400));
...
fn reserve_inventory_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 300));
        System::assert_has_event(
            Event::InventoryReserved { vault_id: id, amount: 300 }.into(),
        );
    });
}
...
fn reserve_inventory_rejects_frozen_vault() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::fund_vault(RuntimeOrigin::root(), vault_id(23), 400));
...
fn release_inventory_moves_balance_from_reserved_to_available() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 400));
        assert_ok!(X3Inventory::release_inventory(RuntimeOrigin::root(), id, 200));
...
fn release_inventory_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 400));
        assert_ok!(X3Inventory::release_inventory(RuntimeOrigin::root(), id, 400));
        System::assert_has_event(
            Event::InventoryReleased { vault_id: id, amount: 400 }.into(),
        );
    });
}
...
fn release_inventory_restores_available_balance_invariant() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 600));
        assert_ok!(X3Inventory::release_inventory(RuntimeOrigin::root(), id, 600));
        let vault = Vaults::<Test>::get(id).unwrap();
...
fn record_pending_out_moves_from_available_to_pending_out() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::record_pending_out(RuntimeOrigin::root(), id, 250));
...
fn record_pending_out_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::record_pending_out(RuntimeOrigin::root(), id, 100));
        System::assert_has_event(
            Event::PendingOutRecorded { vault_id: id, amount: 100 }.into(),
        );
    });
}
...
fn record_pending_out_balance_invariant_holds() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 300));
        assert_ok!(X3Inventory::record_pending_out(RuntimeOrigin::root(), id, 200));
        let vault = Vaults::<Test>::get(id).unwrap();
        assert_eq!(
            vault.available_balance + vault.reserved_balance + vault.pending_out_balance,
            1_000
...
fn confirm_settlement_reduces_pending_out_balance() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::record_pending_out(RuntimeOrigin::root(), id, 300));
        assert_ok!(X3Inventory::confirm_settlement(RuntimeOrigin::root(), id, 300));
...
fn confirm_settlement_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::record_pending_out(RuntimeOrigin::root(), id, 400));
        assert_ok!(X3Inventory::confirm_settlement(RuntimeOrigin::root(), id, 400));
        System::assert_has_event(
            Event::SettlementConfirmed { vault_id: id, amount: 400 }.into(),
        );
    });
}
...
fn fund_vault_increases_available_balance() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::fund_vault(RuntimeOrigin::root(), vault_id(36), 2_000));
...
fn fund_vault_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::fund_vault(RuntimeOrigin::root(), vault_id(37), 1_500));
        System::assert_has_event(
            Event::VaultFunded { vault_id: vault_id(37), amount: 1_500 }.into(),
        );
    });
}
...
fn fund_vault_transitions_frozen_vault_to_active() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::fund_vault(RuntimeOrigin::root(), vault_id(38), 1_500));
        let vault = Vaults::<Test>::get(vault_id(38)).unwrap();
        assert_eq!(vault.status, VaultStatus::Active);
    });
}
...
fn combined_operations_maintain_balance_invariant() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X3Inventory::reserve_inventory(RuntimeOrigin::root(), id, 1_000));
        assert_ok!(X3Inventory::record_pending_out(RuntimeOrigin::root(), id, 500));
        assert_ok!(X3Inventory::release_inventory(RuntimeOrigin::root(), id, 500));
        assert_ok!(X3Inventory::confirm_settlement(RuntimeOrigin::root(), id, 500));
