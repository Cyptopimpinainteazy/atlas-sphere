...
**Prereqfrontend/uisites**: L0-01, L0-02, L0-03 (Genesis, Ledger, Capability Registry)  
...
- Registers operators with reqfrontend/uired capabilities validated against registry
- Manages operator lifecycle (active/inactive/suspended states)
- Enforces capability reqfrontend/uirements before registration
...
type Operator struct {
    ...
    ID string `json:"id" validate:"reqfrontend/uired,alphanum,min=3,max=128"`
    
    // Name is human-readable name
    Name string `json:"name" validate:"reqfrontend/uired,min=1,max=256"`
    ...
    Capabilities []string `json:"capabilities" validate:"reqfrontend/uired,min=1,dive"`
    
    // Status indicates current lifecycle state
    Status OperatorStatus `json:"status" validate:"reqfrontend/uired"`
    ...
    LastUpdatedAt time.Time `json:"lastUpdatedAt" validate:"reqfrontend/uired"`
...
type OperatorValidationResult struct {
    ...
    // MissingCapabilities are reqfrontend/uired but missing
...
type Registry interface {
    ...
    // ValidateOperator checks capability reqfrontend/uirements
...
// ValidateOperator checks operator validity and capability reqfrontend/uirements.
func (ov *OperatorValidator) ValidateOperator(op *Operator) *OperatorValidationResult {
    ...
    if op.ID == "" {
        ...
        result.Errors = append(result.Errors, "ID reqfrontend/uired")
    ...
    if op.Address == "" {
        result.IsValid = false
        result.Errors = append(result.Errors, "Address reqfrontend/uired")
    ...
    if len(op.Capabilities) == 0 {
        result.IsValid = false
        result.Errors = append(result.Errors, "At least one capability reqfrontend/uired")
        return result
    }
...
// TestCapabilityValidation tests capability reqfrontend/uirement enforcement
...
func (m *MockLedgerWriter) Append(entry *ledger.ChangeEntry) error {
    ...
    entry.ID = frontend/uint64(len(m.entries))
    return nil
}
...
func (m *MockLedgerWriter) GetLastID() (frontend/uint64, error) {
    return frontend/uint64(len(m.entries)), nil
}
