# Atlas Sphere Master Documentation Index

This index is the top-level map for project documentation. It consolidates
project docs into a single, predictable structure. Legacy files may remain in
place as stubs that point to the new locations.

## Start Here
- Project overview: [../../README.md](../../README.md)
- Docs home: [../README.md](../README.md)
- Canonical docs index: [../DOCUMENTATION_INDEX.md](../DOCUMENTATION_INDEX.md)

## Getting Started
- Getting started overview: [getting-started/QUICKSTART.md](getting-started/QUICKSTART.md)
- Pre-flight checklist: [getting-started/PRE_FLIGHT_CHECKLIST.md](getting-started/PRE_FLIGHT_CHECKLIST.md)
- Setup completion: [getting-started/SETUP_COMPLETE.md](getting-started/SETUP_COMPLETE.md)

## Desktop Apps
- Desktop complete guide: [desktop/ATLAS_DESKTOP_COMPLETE_GUIDE.md](desktop/ATLAS_DESKTOP_COMPLETE_GUIDE.md)
- Desktop quick reference: [desktop/ATLAS_DESKTOP_QUICK_REFERENCE.md](desktop/ATLAS_DESKTOP_QUICK_REFERENCE.md)
- Desktop verification: [desktop/ATLAS_DESKTOP_VERIFICATION.md](desktop/ATLAS_DESKTOP_VERIFICATION.md)

## Architecture and Specifications
- Architecture overview: [../ARCHITECTURE.md](../ARCHITECTURE.md)
- Tri-VM architecture: [../TRI_VM_ARCHITECTURE.md](../TRI_VM_ARCHITECTURE.md)
- X3 language specification: [../X3_LANGUAGE_SPECIFICATION.md](../X3_LANGUAGE_SPECIFICATION.md)
- X3 language reference: [../X3_LANGUAGE_REFERENCE.md](../X3_LANGUAGE_REFERENCE.md)
- X3Script DSL specification: [../X3SCRIPT_DSL_SPECIFICATION.md](../X3SCRIPT_DSL_SPECIFICATION.md)
- X3Script stdlib reference: [../X3SCRIPT_STDLIB_REFERENCE.md](../X3SCRIPT_STDLIB_REFERENCE.md)
- AI agent API specification: [../AI_AGENT_API_SPECIFICATION.md](../AI_AGENT_API_SPECIFICATION.md)
- Quantum execution whitepaper: [../QUANTUM_EXECUTION_WHITEPAPER.md](../QUANTUM_EXECUTION_WHITEPAPER.md)

## Implementation and Engineering
- Implementation artifacts: [implementation/IMPLEMENTATION_ARTIFACTS.md](implementation/IMPLEMENTATION_ARTIFACTS.md)
- GPU swarm implementation index: [implementation/GPU_SWARM_IMPLEMENTATION_INDEX.md](implementation/GPU_SWARM_IMPLEMENTATION_INDEX.md)
- GPU swarm completion artifact: [implementation/GPU_SWARM_IMPLEMENTATION_COMPLETION_ARTIFACT.md](implementation/GPU_SWARM_IMPLEMENTATION_COMPLETION_ARTIFACT.md)
- Orchestra implementation plan: [implementation/ORCHESTRA_IMPLEMENTATION_PLAN.md](implementation/ORCHESTRA_IMPLEMENTATION_PLAN.md)
- Phase 3 implementation summary: [implementation/P3_IMPLEMENTATION_SUMMARY.md](implementation/P3_IMPLEMENTATION_SUMMARY.md)
- Phase 4 implementation summary: [implementation/P4_IMPLEMENTATION_SUMMARY.md](implementation/P4_IMPLEMENTATION_SUMMARY.md)

## Roadmaps
- Phase 3 quick reference: [roadmaps/P3_QUICK_REFERENCE.md](roadmaps/P3_QUICK_REFERENCE.md)
- Phase 5 EVM GPU kernel roadmap: [roadmaps/P5_DAYS1-5_EVM_GPU_KERNEL_ROADMAP.md](roadmaps/P5_DAYS1-5_EVM_GPU_KERNEL_ROADMAP.md)
- Phase 5 atomic swap roadmap: [roadmaps/P5_DAYS6-10_ATOMIC_SWAP_ORCHESTRATOR_ROADMAP.md](roadmaps/P5_DAYS6-10_ATOMIC_SWAP_ORCHESTRATOR_ROADMAP.md)
- Phase 5 strategic summary: [roadmaps/P5_STRATEGIC_SUMMARY.md](roadmaps/P5_STRATEGIC_SUMMARY.md)

## Operations and Monitoring
- Operations runbook: [operations/OPERATIONS_RUNBOOK.md](operations/OPERATIONS_RUNBOOK.md)
- System monitoring implementation: [operations/SYSTEM_MONITORING_IMPLEMENTATION.md](operations/SYSTEM_MONITORING_IMPLEMENTATION.md)
- Production readiness checkpoint: [operations/PRODUCTION_READINESS_CHECKPOINT.md](operations/PRODUCTION_READINESS_CHECKPOINT.md)
- Cargo build issues: [operations/CARGO_BUILD_ISSUES.md](operations/CARGO_BUILD_ISSUES.md)
- Trial schedule: [operations/TRIAL_SCHEDULE_32_AGENTS.md](operations/TRIAL_SCHEDULE_32_AGENTS.md)

## Communications
- Team communications: [communications/TEAM_COMMUNICATIONS.md](communications/TEAM_COMMUNICATIONS.md)

## Status and Reports
- Project status: [status/PROJECT_STATUS.md](status/PROJECT_STATUS.md)
- Phase 4 progress: [status/PROGRESS_P4.md](status/PROGRESS_P4.md)
- Day 1 status: [status/DAY1_STATUS.md](status/DAY1_STATUS.md)
- Day 2 status: [status/DAY2_STATUS.md](status/DAY2_STATUS.md)
- Day 5 status: [status/DAY5_STATUS.md](status/DAY5_STATUS.md)
- Day 6 status: [status/DAY6_STATUS.md](status/DAY6_STATUS.md)
- Phase 4 executive summary: [status/P4_EXECUTIVE_SUMMARY.md](status/P4_EXECUTIVE_SUMMARY.md)
- Phase 4 midpoint report: [status/P4_MIDPOINT_MILESTONE_REPORT.md](status/P4_MIDPOINT_MILESTONE_REPORT.md)

## Releases and Delivery
- Full shipping ready: [releases/FULL_SHIPPING_READY.md](releases/FULL_SHIPPING_READY.md)
- Ship it checklist: [releases/SHIP_IT.md](releases/SHIP_IT.md)
- Pass A completion: [releases/PASS_A_COMPLETE.txt](releases/PASS_A_COMPLETE.txt)
- Phase 5 execution complete: [releases/PHASE5_EXECUTION_COMPLETE.md](releases/PHASE5_EXECUTION_COMPLETE.md)
- Phase 5 file manifest: [releases/PHASE5_FILE_MANIFEST.md](releases/PHASE5_FILE_MANIFEST.md)
- Phase 5 final delivery summary: [releases/PHASE5_FINAL_DELIVERY_SUMMARY.md](releases/PHASE5_FINAL_DELIVERY_SUMMARY.md)
- Phase 5 quick shipping guide: [releases/PHASE5_QUICK_SHIPPING_GUIDE.md](releases/PHASE5_QUICK_SHIPPING_GUIDE.md)
- Phase 5 verification complete: [releases/PHASE5_VERIFICATION_COMPLETE.md](releases/PHASE5_VERIFICATION_COMPLETE.md)
- Phase 6 build report: [releases/PHASE6_BUILD_REPORT.txt](releases/PHASE6_BUILD_REPORT.txt)

## Security
- Security policy: [../../SECURITY.md](../../SECURITY.md)
- Voir dire specification: [security/VOIR_DIRE_SPECIFICATION.md](security/VOIR_DIRE_SPECIFICATION.md)

## Testing and QA
- AGI test results: [testing/AGI_TEST_RESULTS.md](testing/AGI_TEST_RESULTS.md)

## Docs Maintenance
- Documentation index (legacy root): [docs-maintenance/DOCUMENTATION_INDEX.md](docs-maintenance/DOCUMENTATION_INDEX.md)
- MD consolidation log: [docs-maintenance/MD_CONSOLIDATION_LOG.md](docs-maintenance/MD_CONSOLIDATION_LOG.md)
- MD path updates: [docs-maintenance/MD_PATH_UPDATES.md](docs-maintenance/MD_PATH_UPDATES.md)

## App-Specific Docs
- X3 Intelligence upgrade summary: [apps/x3-intelligence/UPGRADE_SUMMARY.md](apps/x3-intelligence/UPGRADE_SUMMARY.md)
