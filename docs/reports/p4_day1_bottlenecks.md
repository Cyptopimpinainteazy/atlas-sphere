...
- ~~CUDA toolkit is installed but not on PATH; use `/usr/local/cuda-12.2/bin/nvcc`.~~ ✅ RESOLVED
- ~~`ed25519-donna` is not available on PyPI~~ ✅ RESOLVED: Using `cryptography` + `PyNaCl` instead
...
- Cupy + CUDA 11x dependency needs a driver/toolkit alignment; current driver reports CUDA 12.2 runtime, but compilation works.
