...
use atlas_sphere_runtime::{AccountId, AssetId, Balance, opaque::Block};
...
where
	Block: BlockT,
	C: Send + Sync + 'static + ProvideRuntimeApi<Block> + HeaderBackend<Block> + BlockBackend<Block>,
	C::Api: AtlasKernelRuntimeApi<Block, AccountId, Balance, AssetId>,
{
	fn get_canonical_balance(
		...
		api.get_canonical_balance(at, account, asset_id)
			.map_err(|e| jsonrpsee::types::ErrorObjectOwned::owned(1, format!("Runtime API error: {:?}", e), None::<()>))
	}
	...
	fn get_authorized_accounts(
		...
		api.get_authorized_accounts(at)
			.map_err(|e| jsonrpsee::types::ErrorObjectOwned::owned(1, format!("Runtime API error: {:?}", e), None::<()>))
	}
	...
	fn get_authorities(
		...
		api.get_authorities(at)
			.map_err(|e| jsonrpsee::types::ErrorObjectOwned::owned(1, format!("Runtime API error: {:?}", e), None::<()>))
	}
}

/// Create full RPC extensions
...
where
	B: BlockT,
	C: Send + Sync + 'static + ProvideRuntimeApi<B> + HeaderBackend<B> + BlockBackend<B>,
	C::Api: AtlasKernelRuntimeApi<B, AccountId, Balance, AssetId>,
	P: Send + Sync + 'static,
{
	use jsonrpsee::RpcModule;

	let mut module = RpcModule::new(());
	let atlas_kernel = AtlasKernelRpc::<C, B>::new(client);
