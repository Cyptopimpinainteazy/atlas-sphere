...
- **Provider Switching** — Use Muapi, OpenRouter free-image mode, or your own local backend API
- **API Key Management** — Secure key storage in browser localStorage
...
- One of:
  ...
  - An [OpenRouter](https://openrouter.ai/keys) API key (image mode)
  - A local backend implementing the expected endpoints (for full local image/video)
...
Open `http://localhost:5173` in your browser. On first generation/upload, use the modal to choose provider:

- `Muapi` for full hosted model support
- `OpenRouter` for free image generation via `openrouter/free` (video is not supported in OpenRouter mode in this app)
- `Local API` for self-hosted backends at `http://127.0.0.1:8787` by default
...
1. **Muapi** (default): existing behavior with submit + poll flow
2. **OpenRouter**: image generation through `/chat/completions` with image output
3. **Local API**: custom local endpoints for self-hosting

### Muapi Flow

The Muapi provider communicates with [Muapi.ai](https://muapi.ai) using a two-step pattern:
...
- Uses OpenRouter chat completions with image output.
- Default model: `openrouter/free` (configurable in settings).
- In dev, requests are proxied through Vite at `/openrouter`.

### Local API Flow

Expected endpoints on your local backend:
...
Responses should include a media URL in `url` (or `outputs[0]` / `output.url`).
...
- **Muapi.ai / OpenRouter / Local API** — Provider layer for AI generation
