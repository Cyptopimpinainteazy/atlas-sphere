...
export function SearchPanel() {
  ...
  const handleModeChange = (mode: SearchMode) => {
    ...
    setRagResults([]);
    if (mode === 'rag') {
      // keep right sidebar open on RAG so crawl controls are visible
      useIDEStore.getState().setRightSidebar('rag');
    }
  };
      ...
      <div className="p-3 border-b border-ide-border space-y-3">
        ...
        {searchType === 'rag' && (
          <p className="text-[10px] text-ide-text-dim">
            Crawling new URLs lives in the right‑hand <strong>RAG</strong> panel.{' '}
            <button
              className="underline"
              onClick={() => useIDEStore.getState().setRightSidebar('rag')}
            >
              Open RAG panel
            </button>
          </p>
