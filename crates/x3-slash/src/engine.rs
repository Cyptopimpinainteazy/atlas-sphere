...
impl SlashingEngine {
    ...
    pub fn post_bond(
        ...
        self.bond_manager.post_bond(agent_id, amount, current_block, intent_id)
    }
        ...
        for bond_id in expired {
            if let Some(bond) = self.bond_manager.get(bond_id) {
                ...
                let reason = if let Some(iid) = intent_id {
                    SlashReason::BondExpiry { bond_id, intent_id: iid }
                } else {
                ...
                if let Ok(event) = self.execute_slash(
                    agent_id, bond_id, reason, current_block,
                ) {
                    events.push(event);
                }
            }
    ...
    pub fn slash_execution_failure(
        ...
        self.execute_slash(
            agent_id,
            bond_id,
            SlashReason::ExecutionFailure { proof_hash, failure_at },
            current_block,
        )
    }
...
mod tests {
    ...
    fn test_slash_execution_failure() {
        ...
        let bond_id = engine.post_bond(test_agent(), 2_000_000, 100, None).unwrap();

        let event = engine.slash_execution_failure(
            test_agent(),
            bond_id,
            [0xAA; 32],
            42,
    ...
    fn test_slash_state_divergence_is_critical() {
        ...
        let event = engine.slash_state_divergence(
            test_agent(),
            bond_id,
            [0xAA; 32],
            [0xBB; 32],
    ...
    fn test_slash_record_is_permanent() {
        ...
        engine.slash_execution_failure(
            test_agent(),
            bond_id,
            [0xAA; 32],
            42,
