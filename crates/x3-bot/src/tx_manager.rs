...
impl<M: Middleware + 'static> TxManager<M> {
    ...
    pub async fn build_signed_tx(&self, wallet: &LocalWallet, to: Address, data: Bytes, gas_limit: U256) -> Result<Bytes> {
    ...
    pub async fn execute_arb(&self, signed_tx: Bytes) -> Result<TxHash> {
        ...
        info!("Arbitrage transaction submitted: {:?}", pending_tx.tx_hash());
