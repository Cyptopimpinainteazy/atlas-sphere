...
impl BotConfig {
    pub fn from_env() -> Result<Self> {
        let rpc_evm = env::var("RPC_EVM").unwrap_or_default()
            .split(',')
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .collect();
            
        let rpc_svm = env::var("RPC_SVM").unwrap_or_default()
            .split(',')
            .map(|s| s.trim().to_string())
            .filter(|s| !s.is_empty())
            .collect();
        ...
        let config = BotConfig {
            ...
            evm_chain_id: env::var("EVM_CHAIN_ID").unwrap_or_else(|_| "1".to_string()).parse()?,
            arb_threshold_bps: env::var("ARB_THRESHOLD_BPS").unwrap_or_else(|_| "10".to_string()).parse()?,
            slippage_pct: env::var("SLIPPAGE_PCT").unwrap_or_else(|_| "0.5".to_string()).parse()?,
            ...
            prometheus_port: env::var("PROMETHEUS_PORT").unwrap_or_else(|_| "9090".to_string()).parse()?,
        };
