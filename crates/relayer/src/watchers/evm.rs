...
impl EvmHeaderWatcher {
    ...
    pub async fn poll(&self) -> Result<Vec<HeaderInfo>> {
        let current_block = Self::get_block_number(&self.rpc_client, &self.config.rpc_endpoint)
            .await?;
    ...
    async fn get_block_header(&self, block_num: u64) -> Result<BlockHeader> {
        ...
        let state_root_str = block["stateRoot"].as_str()
            .ok_or_else(|| anyhow!("Missing state root"))?;
        let timestamp_str = block["timestamp"].as_str()
