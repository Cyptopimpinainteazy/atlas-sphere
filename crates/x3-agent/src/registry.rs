...
impl AgentRegistry {
    ...
    pub fn link_ephemeral(
        ...
        let agent = self.agents.get_mut(primary_pubkey)
    ...
    pub fn record_execution(
        ...
        let agent = self.agents.get_mut(pubkey)
    ...
    pub fn record_slash(
        ...
        if agent.stats.slash_count >= self.config.critical_slash_threshold {
            ...
            self.events.push(AgentEvent::Deactivated {
                pubkey: *pubkey,
                reason: format!("critical slash threshold reached ({})", agent.stats.slash_count),
                block: current_block,
            });
        }

        // Suspend if bond below minimum
        if agent.current_bond < self.config.min_active_bond
    ...
    pub fn active_count(&self) -> usize {
        self.agents.values().filter(|a| a.status == AgentStatus::Active).count()
    }
...
mod tests {
    ...
    fn test_identity() -> AgentIdentity {
        AgentIdentity { pubkey: [1u8; 32], ephemeral: false }
    }
    ...
    fn test_ephemeral_key_linking() {
        ...
        reg.link_ephemeral(&test_identity().pubkey, ephemeral, 101).unwrap();
    ...
    fn test_slash_deactivation() {
        ...
        reg.record_slash(&test_identity().pubkey, 1_000_000, "test1".to_string(), 110).unwrap();
        assert_eq!(reg.get(&test_identity().pubkey).unwrap().status, AgentStatus::Active);

        reg.record_slash(&test_identity().pubkey, 1_000_000, "test2".to_string(), 120).unwrap();
        assert_eq!(reg.get(&test_identity().pubkey).unwrap().status, AgentStatus::Deactivated);
    }
