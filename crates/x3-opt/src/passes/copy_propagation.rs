...
impl CopyPropagationPass {
    ...
    fn is_identity(&self, op: BinaryOp, lhs: MirValue, rhs: MirValue, literals: &BTreeMap<MirValue, Literal>) -> Option<MirValue> {
        match op {
            // x + 0 = x
            BinaryOp::Add => {
    ...
    fn replace_value(&self, value: MirValue, replacements: &BTreeMap<MirValue, MirValue>) -> MirValue {
    ...
    fn run(&self, module: &mut MirModule) -> OptResult<PassResult> {
                ...
                for block in func.blocks.iter_mut() {
                    ...
                    for stmt in block.statements.iter() {
                        ...
                        if let MirRhs::Binary(op, lhs, rhs) = &stmt.rhs {
                            ...
                            if let Some(identity_operand) = self.is_identity(*op, *lhs, *rhs, &literals) {
                            ...
                            let new_args: Vec<MirValue> = args.iter()
        ...
        if total_changes > 0 {
            Ok(PassResult::with_count(
                total_changes,
                format!("copy_propagation: {} changes in {} iterations", total_changes, iterations),
            ))
        } else {
