use x3_flashloan::{FlashloanPool, BorrowRequest, BorrowPurpose, SettlementEngine};
use x3_proof::{ProofEngine, ProofEngineConfig, AgentIdentity};
...
use x3_flashloan::types::{AssetId, ChainKind, BorrowReceipt};
...
fn main() {
    ...
    println!("Borrow receipt: id={} principal={} premium={}", receipt.id.0, receipt.principal, receipt.premium);

    // Repay
    let repay_amount = receipt.principal + receipt.premium;
    let ok = pool.repay(&receipt.id, repay_amount).expect("repay should succeed");
    println!("Repay ok: {}", ok);
    ...
    let mut engine = ProofEngine::new(
        ...
        100u64,
        AgentIdentity { pubkey: [0x42; 32], ephemeral: false },
        [0xAA; 32],
    );

    // Emit two proofs to simulate execution
    engine.record_state_write(b"k1".to_vec(), None, Some(b"v1".to_vec())).unwrap();
    let p1 = engine.emit_proof(1000, 10, None).unwrap();

    engine.record_state_write(b"k2".to_vec(), None, Some(b"v2".to_vec())).unwrap();
    let p2 = engine.emit_proof(500, 5, None).unwrap();

    let (chain, receipt_info) = engine.finalize().unwrap();
    println!("Generated proof chain hash: {}", hex::encode(chain.chain_hash()));
    println!("Execution receipt: total_gas={}, total_fees={}", receipt_info.total_gas, receipt_info.total_fees);
    ...
    let program = chain.proofs().get(0).map(|p| p.program_hash).unwrap_or([0u8;32]);
    ...
    println!("Tampered proof chain hash: {}", hex::encode(tampered.chain_hash()));

    // 4) File a dispute and adjudicate
    let mut court = Court::new(CourtConfig::default());
    let dispute_id = court.file_dispute(
        DisputeType::ExecutionDivergence { proof_chain_hash: chain.chain_hash() },
        AgentIdentity { pubkey: [0x42; 32], ephemeral: false },
        200u64,
    ).unwrap();

    let verdict = court.adjudicate(dispute_id, &chain, &tampered, 205u64).unwrap();
    ...
    let srec = settlement_engine.settle(&receipt, repay_amount, &ProofEngine::new(ProofEngineConfig::default(), 100, AgentIdentity { pubkey: [0xFF;32], ephemeral: true }, [0x00;32])).unwrap();
    println!("Settlement record: id={} status={:?} proof_hash={}", srec.flashloan_id.0, srec.status, srec.proof_hash);
}
