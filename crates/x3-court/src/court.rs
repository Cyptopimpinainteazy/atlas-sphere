...
use x3_proof::verifier::{ProofVerifier, ComparisonResult};
...
impl Court {
    ...
    pub fn adjudicate(
        ...
        let dispute = self.docket.get_mut(dispute_id)
    ...
    pub fn adjudicate_double_execution(
        ...
        let _dispute = self.docket.get_mut(dispute_id)
            .ok_or(CourtError::DisputeNotFound(dispute_id))?;
...
mod tests {
    ...
    fn make_chain(block: u64, program: Hash256, proofs: Vec<(Hash256, Hash256, u64)>) -> ProofChain {
    ...
    fn test_matching_chains_not_guilty() {
        ...
        let chain = make_chain(100, [0xAA; 32], vec![
            ([0u8; 32], [1u8; 32], 100),
        ]);
    ...
    fn test_divergent_chains_guilty() {
        ...
        let original = make_chain(100, [0xAA; 32], vec![
            ([0u8; 32], [1u8; 32], 100),
        ]);
        let replay = make_chain(100, [0xAA; 32], vec![
