...
impl TpsTracker {
    ...
    async fn write_metrics(&self, metrics: &[TransactionStats]) -> Result<(), Box<dyn std::error::Error>> {
        if metrics.is_empty() {
            return Ok(());
        }
        ...
        self.http_client
            .post(&format!("{}/write?db={}", self.config.influx_url, self.config.influx_db))
            .header("Authorization", format!("Bearer {}", self.config.influx_token))
            .body(data)
