...
async fn execute_send(args: SendArgs) -> Result<()> {
    ...
    let keyfile = args.keyfile
        ...
        .ok_or_else(|| CliError::Config("No keyfile specified. Use --keyfile or set X3_KEYFILE".into()))?;
    ...
    // Parse amount
    let _amount: u128 = args.amount.parse()
...
async fn execute_comit(args: ComitTxArgs) -> Result<()> {
    ...
    if args.evm.is_none() && args.svm.is_none() {
        return Err(CliError::Config("At least one of --evm or --svm is required".into()));
    }
    ...
    if args.dry_run {
        ...
        println!("  EVM payload: {} bytes", comit.evm_payload.as_ref().map(|p| p.len()).unwrap_or(0));
        println!("  SVM payload: {} bytes", comit.svm_payload.as_ref().map(|p| p.len()).unwrap_or(0));
    ...
    if let Some(evm) = result.evm_receipt {
        println!("  EVM: {} (gas: {})", 
            if evm.success { "Success".green() } else { "Failed".red() },
            evm.gas_used
        );
    }
    ...
    if let Some(svm) = result.svm_receipt {
        println!("  SVM: {} (compute: {})", 
            if svm.success { "Success".green() } else { "Failed".red() },
...
fn default_keyfile() -> Option<PathBuf> {
    std::env::var("X3_KEYFILE").ok().map(PathBuf::from)
        .or_else(|| {
            dirs::home_dir().map(|h| h.join(".x3").join("default.key"))
        })
}
