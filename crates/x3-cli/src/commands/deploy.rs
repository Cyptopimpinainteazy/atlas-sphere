...
pub async fn execute(args: DeployArgs) -> Result<()> {
    ...
    println!("  {} Contract bytecode: {} bytes", "→".blue(), bytecode.len());
    ...
    let private_key = args.private_key
        ...
        .ok_or_else(|| {
            CliError::Config("Private key required. Use --private-key or X3_PRIVATE_KEY env var".to_string())
        })?;
...
async fn simulate_deploy(endpoint: &str, bytecode: &[u8]) -> Result<()> {
    ...
    let gas = client.evm_estimate_gas("0x0000000000000000000000000000000000000000", bytecode).await?;
    
    println!("  {} Estimated gas: {}", "✓".green(), gas);
    println!("{} Simulation complete - deployment would succeed", "✓".green());
