...
async fn execute_block(args: BlockArgs) -> Result<()> {
    ...
    println!("  State Root: 0x{}", hex::encode(&header.state_root.0[..16]));
    println!("  Extrinsics: 0x{}", hex::encode(&header.extrinsics_root.0[..16]));
...
async fn execute_balance(args: BalanceArgs) -> Result<()> {
    ...
    if let Ok(info) = client.account_info(&args.address).await {
        ...
        println!("  Native Balance: {}", atlas_sdk::utils::format_balance(info.native_balance, 12));
