...
use atlas_sdk::{AtlasClient, Sr25519Signer, Signer};
...
async fn execute_info(args: InfoArgs) -> Result<()> {
    ...
    match client.account_info(&args.address).await {
        Ok(info) => {
            ...
            println!("  Authorized: {}", if info.is_authorized { "Yes".green() } else { "No".yellow() });
            
            if !info.asset_balances.is_empty() {
                println!();
                println!("  Asset Balances:");
                for (asset_id, balance) in &info.asset_balances {
                    println!("    {}: {}", hex::encode(&asset_id.0[..4]), format_balance(*balance));
                }
            }
        }
        Err(e) => {
