...
pub async fn execute(args: DocgenArgs) -> Result<()> {
    ...
    println!("{} Generating documentation for: {}", "→".blue(), project.config.name);
    ...
    match forge_result {
        ...
        Err(_) => {
            ...
            println!("{} forge doc not available, generating basic docs...", "!".yellow());
            generate_basic_docs(&project, &sol_files, &output_dir, &args)?;
        }
    }
    
    println!("{} Documentation written to: {}", "✓".green(), output_dir.display());
...
fn generate_basic_docs(
    ...
    for file in files {
        ...
        let name = file.file_stem().map(|s| s.to_string_lossy()).unwrap_or_default();
