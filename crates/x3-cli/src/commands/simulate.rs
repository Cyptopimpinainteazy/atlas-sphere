...
pub async fn execute(args: SimulateArgs) -> Result<()> {
    ...
    let endpoint = if let Ok(project) = Project::load_current() {
        project.config.network.get_endpoint(args.network.as_deref()).to_string()
    } else {
        args.network.clone().unwrap_or_else(|| atlas_sdk::DEFAULT_HTTP_ENDPOINT.to_string())
    };
    ...
    println!("  Prepare root: 0x{}", hex::encode(&comit.prepare_root.0[..8]));
    ...
    if let Some(ref evm) = comit.evm_payload {
        ...
        match client.evm_estimate_gas("0x0000000000000000000000000000000000000000", evm).await {
            Ok(gas) => {
    ...
    println!("  x3 tx submit --evm <calldata> --network {}", 
