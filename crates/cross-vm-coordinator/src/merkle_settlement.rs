...
mod coordinator_merkle_tests {
    ...
    fn test_fast_claim_non_bridge_without_proof_falls_through_to_session_lookup() {
        ...
        assert!(
            !err.to_string().contains("failed verification"),
            "expected no-proof non-bridge fast path to avoid verification-failure marker"
        );
    }
    ...
    fn test_fast_and_slow_mismatched_non_bridge_share_error_class_markers() {
        ...
        assert!(
            ...
            "expected fast mismatched non-bridge path to fail before session-not-found lookup"
        );
        assert!(
            !slow_msg.contains("Session not found"),
            "expected slow mismatched non-bridge path to fail before session-not-found lookup"
        );
    }
    ...
    fn test_fast_and_slow_mismatched_freshness_bridge_share_error_class_markers() {
        ...
        assert!(
            !fast_msg.contains("Session not found"),
            "expected fast mismatched freshness path to fail before session-not-found lookup"
        );
        assert!(
            !slow_msg.contains("Session not found"),
            "expected slow mismatched freshness path to fail before session-not-found lookup"
        );
    }
    ...
    fn test_slow_claim_bridge_without_proof_falls_through_to_session_lookup() {
        ...
        assert!(
            !err.to_string().contains("bridge verification failed"),
            "expected no-proof bridge slow path to avoid bridge-verification failure marker"
        );
    }
    ...
    #[test]
    fn test_fast_and_slow_freshness_mismatch_precedence_share_error_class_markers() {
        let mut validators = BTreeMap::new();
        validators.insert([1u8; 32], vec![7u8; 32]);
