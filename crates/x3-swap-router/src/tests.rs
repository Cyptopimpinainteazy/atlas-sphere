...
async fn optimizer_picks_highest_output_lowest_gas() {
    ...
    let quotes = vec![
        QuoteResult {
            route: SwapRoute { hops: vec![hop(500)], amount_in: p.amount_in, estimated_output: U256::from(950_000u64), gas_estimate: U256::from(25_000u64) },
            estimated_output: U256::from(950_000u64),
            gas_cost: U256::from(25_000u64),
        },
        QuoteResult {
            route: SwapRoute { hops: vec![hop(700)], amount_in: p.amount_in, estimated_output: U256::from(950_000u64), gas_estimate: U256::from(30_000u64) },
            estimated_output: U256::from(950_000u64),
            gas_cost: U256::from(30_000u64),
        },
        QuoteResult {
            route: SwapRoute { hops: vec![hop(400)], amount_in: p.amount_in, estimated_output: U256::from(940_000u64), gas_estimate: U256::from(20_000u64) },
...
async fn optimizer_errors_when_no_route_meets_min_out() {
    ...
    let quotes = vec![QuoteResult {
        route: SwapRoute { hops: vec![hop(300)], amount_in: p.amount_in, estimated_output: U256::from(900_000u64), gas_estimate: U256::from(21_000u64) },
...
async fn gas_optimizer_returns_nonzero_defaults() {
    ...
    let route = SwapRoute { hops: vec![hop(100)], amount_in: U256::from(1), estimated_output: U256::from(1), gas_estimate: U256::zero() };
...
async fn slippage_controller_rejects_zero_tolerance() {
    ...
    let route = SwapRoute { hops: vec![hop(100)], amount_in: p.amount_in, estimated_output: p.min_amount_out, gas_estimate: U256::zero() };
...
async fn fee_calculator_sums_total() {
    ...
    let route = SwapRoute { hops: vec![hop(100)], amount_in: U256::from(1_000_000u64), estimated_output: U256::from(1_000_000u64), gas_estimate: U256::from(10_000u64) };
...
async fn executor_marks_success_with_nonempty_inputs() {
    ...
    let route = SwapRoute { hops: vec![hop(100)], amount_in: U256::from(1), estimated_output: U256::from(1), gas_estimate: U256::from(10_000u64) };
    let gas = ChainGasParams { gas_price: U256::from(1_000_000_000u64), gas_limit: U256::from(25_000u64) };
    let params = SlippageProtectedParams { params: sample_params(), slippage_bps: 50 };
    let res = exec.execute_swap_bundle(&route, &gas, &params).await.unwrap();
    assert!(res.success);
...
async fn mev_overhead_error_propagates() {
    ...
    let route = SwapRoute { hops: vec![hop(100)], amount_in: U256::from(1), estimated_output: U256::from(1), gas_estimate: U256::from(1) };
