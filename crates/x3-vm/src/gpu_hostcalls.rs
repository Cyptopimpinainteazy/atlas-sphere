...
fn load_pipeline_lib() -> Result<StreamPipelineLib, String> {
    ...
    unsafe {
        ...
        let print_info: libloading::Symbol<PipelinePrintInfoFn> = lib
            .get(b"pipeline_print_info")
...
fn load_keccak256_lib() -> Result<Keccak256Lib, String> {
    ...
    unsafe {
        ...
        Ok(Keccak256Lib { batch: batch_fn, _cuda_lib: CudaLib { _lib: lib } })
    }
}
...
impl GpuHostcalls {
    ...
    pub fn register_all(&self, registry: &mut HostcallRegistry) {
...

        {
            ...
            registry.register(gpu_hostcall_ids::GPU_SHA256_BATCH, "gpu_sha256_batch", 2,
                move |args| Self::handle_sha256_batch(&lib, args));
        }
        {
            let lib = ed25519.clone();
            registry.register(gpu_hostcall_ids::GPU_ED25519_VERIFY, "gpu_ed25519_verify", 2,
                move |args| Self::handle_ed25519_verify(&lib, args));
        }
        {
            let lib = sha256.clone();
            registry.register(gpu_hostcall_ids::GPU_POH_CHAIN, "gpu_poh_chain", 3,
                move |args| Self::handle_poh_chain(&lib, args));
        }
        {
            let lib = pipeline.clone();
            registry.register(gpu_hostcall_ids::GPU_SHA256_STREAMED, "gpu_sha256_streamed", 3,
                move |args| Self::handle_sha256_streamed(&lib, args));
        }
        {
            let has_any = sha256.is_some() || ed25519.is_some() || pipeline.is_some()
                || keccak256.is_some() || secp256k1.is_some();
            registry.register(gpu_hostcall_ids::GPU_DEVICE_COUNT, "gpu_device_count", 0,
                move |_args| Ok(Some(Value::I64(if has_any { 3 } else { 0 }))));
        }
        {
            let lib = pipeline.clone();
            registry.register(gpu_hostcall_ids::GPU_BENCHMARK, "gpu_benchmark", 2,
                move |args| Self::handle_benchmark(&lib, args));
        }
        {
            let lib = keccak256.clone();
            registry.register(gpu_hostcall_ids::GPU_KECCAK256_BATCH, "gpu_keccak256_batch", 2,
    ...
    pub fn register_on_vm(&self, vm: &mut crate::vm::VM) {
        ...
        // 0xD4: gpu_device_count
        {
            ...
            vm.register_hostcall(
                ...
                0,
                move |_args| {
                    Ok(Some(Value::I64(if has_any { 3 } else { 0 })))
                },
            );
        }
    ...
    fn handle_ed25519_verify(
        ...
        let ret =
            unsafe { (lib.verify_multi_gpu)(sigs.as_ptr(), count, results.as_mut_ptr()) };
    ...
    fn handle_keccak256_batch(
        ...
        let inputs = match &args[0] {
            Value::Bytes(b) => b,
            _ => return Err(VMError::without_ip(VMErrorKind::TypeMismatch(
                "Bytes".into(), format!("{:?}", args[0]),
            ))),
        };
        let count = args[1].as_i64()? as i32;
        ...
        if ret != 0 {
            return Err(VMError::without_ip(VMErrorKind::HostcallError(format!(
                "gpu_keccak256_batch: CUDA error code {}", ret
            ))));
        }
        Ok(Some(Value::Bytes(output)))
    }
    ...
    fn handle_secp256k1_verify(
        ...
        let ret = unsafe {
            (lib.verify_multi_gpu)(
                u1.as_ptr(), u2.as_ptr(), pk.as_ptr(),
                count, output.as_mut_ptr(),
            )
        };
        if ret != 0 {
            return Err(VMError::without_ip(VMErrorKind::HostcallError(format!(
                "gpu_secp256k1_verify: CUDA error code {}", ret
            ))));
        }
        Ok(Some(Value::Bytes(output)))
    }
