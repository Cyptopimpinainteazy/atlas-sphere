...
fn gpu_opcodes_charge_gas() {
    ...
    assert_eq!(result.gas_used, 12, "GPU device count should cost 10 gas + 2 for ret");
}
...
fn gpu_sha256_batch_empty() {
    ...
    let code = vec![
        Opcode::GpuSha256Batch.to_byte(), 0x00, 0x01, 0x02, // r0 = gpu_sha256_batch(r1, r2)
        Opcode::Ret.to_byte(), 0x00,
    ];
...
fn gpu_ed25519_verify_empty() {
    let code = vec![
        Opcode::GpuEd25519Verify.to_byte(), 0x00, 0x01, 0x02,
        Opcode::Ret.to_byte(), 0x00,
    ];
...
fn gpu_poh_chain_empty() {
    let code = vec![
        Opcode::GpuPohChain.to_byte(), 0x00, 0x01, 0x02, 0x03,
        Opcode::Ret.to_byte(), 0x00,
    ];
...
fn gpu_multi_op_sequence() {
    let code = vec![
        ...
        Opcode::GpuDeviceCount.to_byte(), 0x0A,
        // r0 = gpu_sha256_batch(r1, r2) → 4 bytes
        Opcode::GpuSha256Batch.to_byte(), 0x00, 0x01, 0x02,
        // r5 = gpu_ed25519_verify(r3, r4) → 4 bytes
        Opcode::GpuEd25519Verify.to_byte(), 0x05, 0x03, 0x04,
        // return r10
        Opcode::Ret.to_byte(), 0x0A,
    ];
...
fn gpu_sha256_batch_real_data() {
    ...
    match result.value {
        Some(Value::Bytes(hashes)) => {
            assert_eq!(hashes.len(), count * 32, "should return {} bytes", count * 32);
            println!("[GPU Test] SHA-256 batch first hash: {:02x?}", &hashes[..32]);
            // Verify hashes are non-zero (actual correctness tested in CUDA unit tests)
            assert!(hashes.iter().any(|&b| b != 0), "hashes should not be all zeros");
        }
