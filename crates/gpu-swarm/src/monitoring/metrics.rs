...
impl MetricsCollector {
    ...
    pub fn new_with_registry(registry: Registry) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // Task Metrics
        let task_queue_depth = Gauge::new(
            "gpu_task_queue_depth",
            "Current task queue depth",
        )?;
        registry.register(Box::new(task_queue_depth.clone()))?;

        let task_submitted_total = Counter::new(
            "gpu_task_submitted_total",
        ...
        let task_failed_total = Counter::new(
            "gpu_task_failed_total",
            "Total tasks failed",
        )?;
        registry.register(Box::new(task_failed_total.clone()))?;

        let task_execution_time = Histogram::with_opts(
            HistogramOpts::new("gpu_task_execution_time_seconds", "Task execution time in seconds")
        ...
        let task_execution_time_by_type = HistogramVec::new(
            HistogramOpts::new("gpu_task_execution_time_by_type_seconds", "Task execution time by type")
        ...
        let coordinator_request_latency = Histogram::with_opts(
            HistogramOpts::new("coordinator_request_latency_ms", "Coordinator request latency in milliseconds")
        ...
        let slashing_events_total = Counter::new(
        ...
        let database_query_latency = HistogramVec::new(
            HistogramOpts::new("database_query_latency_ms", "Database query latency in milliseconds")
