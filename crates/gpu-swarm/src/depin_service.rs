...
use crate::jobs::{JobType, SwarmJob, JobOutput};
...
impl DepinService {
    ...
    pub async fn accept_order(
        ...
        self.active_rentals.write().await.insert(order.order_id, rental);
        Ok(())
    }
    ...
    pub async fn preempt(&self, order_id: &[u8; 16]) -> Result<(), DepinError> {
        ...
        let rental = rentals.get_mut(order_id).ok_or(DepinError::RentalNotFound)?;
...
mod tests {
    ...
    async fn reject_tier_mismatch() {
        ...
        let result = service
            .accept_order(order, 8192, GpuTier::DataCenter)
            .await;
