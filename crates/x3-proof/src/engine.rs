...
impl ProofEngine {
    ...
    fn apply_diff_to_snapshot(&mut self, diff: &StateDiff) {
        match &diff.new_value {
            Some(val) => {
                // Upsert
                match self.state_snapshot.binary_search_by(|(k, _)| k.cmp(&diff.key)) {
                    Ok(idx) => self.state_snapshot[idx].1 = val.clone(),
                    Err(idx) => self.state_snapshot.insert(idx, (diff.key.clone(), val.clone())),
                }
            }
            None => {
                // Delete
                if let Ok(idx) = self.state_snapshot.binary_search_by(|(k, _)| k.cmp(&diff.key)) {
...
mod tests {
    ...
    fn test_atomic_proof() {
        ...
        engine.record_state_write(vec![1], None, Some(vec![10])).unwrap();
        engine.record_state_write(vec![2], None, Some(vec![20])).unwrap();
