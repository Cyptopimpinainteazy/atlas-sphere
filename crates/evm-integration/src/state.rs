	...
	pub fn compute_state_root(&self) -> [u8; 32] {
		...
		#[cfg(feature = "std")]
		{
			...
			fn rlp_account(nonce: U256, balance: U256, storage_root: [u8; 32], code_hash: [u8; 32]) -> Vec<u8> {
				...
				s.append(&nonce);
				s.append(&balance);
				s.append(&EthH256::from(storage_root));
				s.append(&EthH256::from(code_hash));
				s.out().to_vec()
			}
...
/// Frontier-compatible state backend over the in-memory `EvmStateDb`.
///
/// Implements Frontier's (fp-evm) backend traits so the `evm` stack executor can
/// read and mutate accounts, code, and storage.
