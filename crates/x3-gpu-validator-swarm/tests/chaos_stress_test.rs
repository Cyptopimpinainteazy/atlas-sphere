...
mod chaos_tests {
    ...
    impl ChaosStressTest {
        ...
        pub async fn run(&self) -> MetricsSnapshot {
            ...
            println!("\n{}", "=".repeat(60));
            println!("🔥 CHAOS STRESS TEST: {} TPS for {} seconds", 
                     self.config.target_tps, self.config.duration_secs);
            println!("   Chaos: {}", if self.config.chaos_enabled { "ENABLED" } else { "disabled" });
            ...
            for worker_id in 0..num_workers {
                ...
                let handle = tokio::spawn(async move {
                    while !should_stop.load(Ordering::Acquire) {
                        ...
                        let result = Self::execute_task_with_chaos(
                            &config,
                            worker_id,
                            task_start
                        ).await;
                        ...
                        match result {
                            ...
                            Err(TaskError::Failure(msg)) => {
                                failed.fetch_add(1, Ordering::Relaxed);
                                errors.lock().push(format!("[Worker {}] {}", worker_id, msg));
                            }
                        }
    ...
    async fn execute_task_with_chaos(
        ...
        if config.chaos_enabled {
            ...
            if random_val < config.failure_rate {
                return Err(TaskError::Failure(
                    format!("Injected failure (worker {})", worker_id)
                ));
            }
        }
        ...
        fn print_status(&self, current_tps: f64, submitted: u64, completed: u64, failed: u64, elapsed: f64) {
    ...
    async fn chaos_test_baseline_10k_tps_clean() {
        ...
        assert!(metrics.current_tps >= 9_000.0, 
                "Failed to achieve 9K TPS, got {:.0}", metrics.current_tps);
    }
    ...
    async fn chaos_test_30k_tps_clean() {
        ...
        assert!(metrics.current_tps >= 27_000.0, 
                "Failed to achieve 27K TPS, got {:.0}", metrics.current_tps);
    }
    ...
    async fn chaos_test_65k_tps_clean() {
        ...
        assert!(metrics.current_tps >= 50_000.0, 
                "Failed to achieve 50K TPS, got {:.0}", metrics.current_tps);
    }
    ...
    async fn chaos_test_seek_breaking_point() {
        for target_tps in &[100_000u64, 250_000, 500_000, 750_000, 1_000_000] {
            ...
            println!("✓ Achieved: {:.0} TPS ({:.1}% of target)", metrics.current_tps, efficiency);
            
            if efficiency < 50.0 {
                println!("❌ BREAKING POINT REACHED at {:.0}K TPS", *target_tps as f64 / 1000.0);
                println!("   System can sustain up to ~{:.0}K TPS", metrics.current_tps / 1000.0);
                break;
            }
        }
    }
