...
mod tests {
        ...
        async fn simulate_load(&mut self) {
            ...
            while test_start.elapsed() < test_duration {
                ...
                if elapsed.as_micros() < iteration_time as u128 {
                    tokio::time::sleep(
                        Duration::from_micros(iteration_time as u64)
                            - elapsed
                    ).await;
                }
            }
    ...
    async fn test_stress_with_real_time_tps_1k() {
        ...
        println!("  Latency avg/p50/p95/p99/peak: {:.2}/{:.2}/{:.2}/{:.2}/{:.2}ms", 
            avg, p50, p95, p99, peak);
    ...
    async fn test_sustained_stress_with_window_analysis() {
        ...
        assert!(test.tasks_completed > 10000, "Should complete 10K+ tasks in 30s at 3K TPS");
        assert!(p99 < 50.0, "P99 should be reasonable (< 50ms)");
        assert!(test.tasks_failed == 0, "Should not fail tasks in normal operation");
    }
    ...
    async fn test_tps_stability_across_windows() {
        ...
        while test_start.elapsed() < test_duration {
            ...
            if last_window_time.elapsed() >= Duration::from_secs(5) {
                ...
                let window_tps = tasks_this_window as f64 / last_window_time.elapsed().as_secs_f64();
            ...
            if elapsed.as_micros() < iteration_time as u128 {
                tokio::time::sleep(
                    Duration::from_micros(iteration_time as u64) - elapsed
                ).await;
            }
        }

        let avg_window_tps: f64 = window_tps_values.iter().sum::<f64>() / window_tps_values.len() as f64;
        let max_window_tps = window_tps_values.iter().copied().fold(0.0, f64::max);
        let min_window_tps = window_tps_values.iter().copied().fold(f64::INFINITY, f64::min);
        ...
        assert!(variance < 50.0, "TPS should be relatively stable across windows");
    }
}
