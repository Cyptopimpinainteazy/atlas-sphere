...
use crate::deterministic::{DeterministicEngine, DeterministicTask, ExecutionResult, ExecutionMode};
...
impl Validator {
    ...
    pub fn initialize(&self) -> SwarmResult<()> {
        ...
        self.engine.set_cpu_verification(self.config.verification.cpu_verification_enabled);
        self.engine.set_replay_mode(self.config.verification.replay_mode_enabled);
        self.engine.set_hash_algorithm(HashAlgorithm::Keccak256);

        // Register health checks
        self.health.register("engine".to_string(), || {
            crate::metrics::HealthCheck {
    ...
    pub fn process_task(&self, task: DeterministicTask) -> ExecutionResult {
        ...
        if self.quarantine.is_quarantined(&self.validator_id) {
            return ExecutionResult::error(
                task.task_id,
                "Validator is quarantined".to_string(),
            );
        }
        ...
        self.metrics.record_task(&self.validator_id, latency_ms, success, divergent);
