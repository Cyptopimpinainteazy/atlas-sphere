...
impl PaymentSystem {
    ...
    pub fn register_provider(
        ...
        if providers.contains_key(&provider_id) {
            return Err(SwarmError::InvalidInput("Provider already registered".to_string()));
        }
    ...
    pub fn confirm_settlement(&self, settlement_id: &str, tx_hash: String) -> SwarmResult<()> {
        ...
        if let Some(settlement) = settlements.iter_mut().find(|s| s.settlement_id == settlement_id) {
...
mod tests {
    ...
    fn test_provider_registration() {
        ...
        let result = payment.register_provider(
            "provider1".to_string(),
            "0x1234".to_string(),
            1000000,
        );
