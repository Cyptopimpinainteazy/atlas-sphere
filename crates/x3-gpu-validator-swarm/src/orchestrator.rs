...
impl SwarmOrchestrator {
    ...
    pub fn register_validator(&self, validator: Arc<Validator>) {
        ...
        self.metrics.update_validator_count(total, active, quarantined);
    }
    ...
    pub fn get_quarantined_validators(&self) -> usize {
        self.validators.read().values()
            .filter(|v| v.get_quarantine_status().map(|s| s.is_quarantined).unwrap_or(false))
            .count()
    }
    ...
    fn select_validator(&self) -> Option<String> {
        ...
        let available: Vec<_> = validators.iter()
            .filter(|(_, v)| {
                v.state() == crate::validator::ValidatorState::Running &&
                !v.get_quarantine_status().map(|s| s.is_quarantined).unwrap_or(false)
            })
            .map(|(id, v)| {
        ...
        match self.assignment_strategy {
            ...
            AssignmentStrategy::LeastLoaded => {
                ...
                available.into_iter()
                    .min_by(|a, b| {
                        a.1.total_tasks.cmp(&b.1.total_tasks)
                    })
                    .map(|(id, _)| id)
            }
