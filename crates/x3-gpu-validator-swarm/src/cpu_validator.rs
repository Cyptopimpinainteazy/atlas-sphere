...
impl CpuValidator {
    ...
    pub fn get_metrics(&self) -> CpuValidatorMetrics {
        ...
        CpuValidatorMetrics {
            ...
            success_rate: if processed > 0 { successful as f64 / processed as f64 } else { 0.0 },
...
impl EasyCpuValidator {
    ...
    pub fn hash(&self, data: &[u8]) -> HashOutput {
        let result = self.inner.process_task(
            vec![data.to_vec()],
...
mod tests {
    ...
    fn test_cpu_batch() {
        let validator = EasyCpuValidator::default();
        let hashes = validator.hash_batch(vec![
            b"hello".to_vec(),
            b"world".to_vec(),
            b"test".to_vec(),
        ]);
    ...
    fn test_standalone_batch() {
        let hashes = validate_cpu_batch(vec![
            b"a".to_vec(),
            b"b".to_vec(),
            b"c".to_vec(),
        ]);
