...
fn run_benchmark() {
    ...
    println!("Hardware: {} cores, {} MB memory, GPU: {}",
        hw_info.cpu_cores,
        hw_info.memory_mb,
        if hw_info.gpu_available { "available" } else { "not available" }
    );
    
    // Benchmark different configurations
    let configs = vec![
        ("Keccak256", HashAlgorithm::Keccak256, vec![1, 10, 100, 1000, 10000]),
        ("SHA256", HashAlgorithm::Sha256, vec![1, 10, 100, 1000, 10000]),
    ];
    
    for (algo_name, algo, batch_sizes) in configs {
        println!("\nBenchmarking {}:", algo_name);
        println!("{:<12} {:<12} {:<15} {:<15} {:<12}", "Batch", "Time (ms)", "Avg (us)", "Ops/sec", "Success");
        println!("{}", "-".repeat(66));
        ...
        for batch_size in batch_sizes {
            ...
            let success_rate = if result.verification == x3_gpu_validator_swarm::crypto::VerificationResult::Valid {
                100.0
            } else {
                0.0
            };
    ...
    let avg_throughput = if total_time > 0.0 { total_ops as f64 / (total_time / 1000.0) } else { 0.0 };
...
fn show_latest_report() {
    ...
    println!("  GPU: {}", if report.hardware.gpu_available { "available" } else { "not available" });
    println!();
    println!("Summary:");
    println!("  Total operations: {}", report.summary.total_operations);
    println!("  Total time: {:.2} ms", report.summary.total_time_ms);
    println!("  Avg throughput: {:.0} ops/sec", report.summary.avg_throughput);
    println!("  Peak throughput: {:.0} ops/sec", report.summary.peak_throughput);
    println!();
    println!("Results:");
