...
impl DeterministicEngine {
    ...
    pub fn set_cpu_verification(&self, enabled: bool) {
        self.cpu_verification_enabled.store(enabled, Ordering::SeqCst);
    }
    ...
    pub fn execute(&self, task: DeterministicTask) -> ExecutionResult {
        ...
        let result = match mode {
            ExecutionMode::GpuOnly => {
                self.execute_gpu(&task, algorithm)
            }
            ...
            Err(e) => {
                ...
                if mode != ExecutionMode::CpuFallback && mode != ExecutionMode::CpuOnly {
                    ...
                    match cpu_result {
                        ...
                        Err(cpu_e) => {
                            ExecutionResult::error(task.task_id, cpu_e.to_string())
                        }
                    }
                } else {
    ...
    fn execute_gpu(&self, task: &DeterministicTask, algorithm: HashAlgorithm) -> SwarmResult<ExecutionResult> {
        // Simulate GPU execution (in production, this would call CUDA/OpenCL)
        let outputs: Vec<HashOutput> = task.inputs
            .iter()
    ...
    fn execute_with_verification(&self, task: &DeterministicTask, algorithm: HashAlgorithm) -> SwarmResult<ExecutionResult> {
        ...
        if cpu_verification_enabled {
            let cpu_outputs: Vec<HashOutput> = task.inputs
                .iter()
                .map(|input| crate::crypto::compute_hash(&algorithm, input))
                .collect();
            ...
            if needs_verification && gpu_outputs != cpu_outputs {
                ...
                if replay_mode_enabled {
                    let replay_outputs: Vec<HashOutput> = task.inputs
                        .iter()
                        .map(|input| crate::crypto::compute_hash(&algorithm, input))
                        .collect();
                    ...
                    if gpu_outputs == replay_outputs {
                        ...
                        return Ok(ExecutionResult::divergent(task.task_id.clone(), gpu_outputs, 0));
                    }
                }
    ...
    /// Execute on CPU only
    fn execute_cpu(&self, task: &DeterministicTask, algorithm: HashAlgorithm) -> SwarmResult<ExecutionResult> {
        let outputs: Vec<HashOutput> = task.inputs
            .iter()
            .map(|input| crate::crypto::compute_hash(&algorithm, input))
            .collect();
    ...
    pub fn get_stats(&self) -> EngineStats {
        EngineStats {
            ...
            replay_verifications: AtomicU32::new(self.stats.replay_verifications.load(Ordering::SeqCst)),
        }
    }
