...
impl BaseAdapter {
    ...
    async fn rpc_call(&self, method: &str, params: &str) -> AdapterResult<Vec<u8>> {
        ...
        let response_bytes = crate::rpc_http::post_json(&url, &body)
            .await
            .map_err(|e| crate::error::ExternalChainError::RpcError(format!("HTTP error: {}", e)))?;
    ...
    fn parse_hex_u256(hex_str: &str) -> AdapterResult<U256> {
        ...
        let bytes = hex::decode(&padded)
            .map_err(|e| crate::error::ExternalChainError::ParseError(format!("hex decode: {}", e)))?;
    ...
    fn extract_result(response: &[u8]) -> AdapterResult<String> {
        ...
        if text.contains("\"error\"") {
            return Err(crate::error::ExternalChainError::RpcError(
                format!("RPC error: {}", text)
            ));
        }
...
impl ChainAdapter for BaseAdapter {
    ...
    async fn send_message(&self, message: ChainMessage) -> AdapterResult<H256> {
        ...
        let calldata = Self::encode_send_message(
            message.recipient,
            &message.payload,
            message.gas_limit,
        );
    ...
    async fn receive_messages(&self) -> AdapterResult<Vec<ChainMessage>> {
        ...
        let sent_message_topic = "0xcb0f7ffd78f9aee47a248fae8db181db6eee833039123e026dcbff529522e52a";
...
mod tests {
    ...
    fn test_encode_send_message() {
        let calldata = BaseAdapter::encode_send_message(
            H160::zero(),
            &[1, 2, 3],
            200_000,
        );
