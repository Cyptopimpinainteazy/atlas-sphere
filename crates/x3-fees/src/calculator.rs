...
mod tests {
    ...
    fn test_x3_cheaper_than_external() {
        ...
        assert!(x3 < ext, "X3 fee {} should be less than external {}", x3, ext);
        assert!(savings > 0);
    }
    ...
    fn test_profitability() {
        ...
        let (profitable, _) = calc.estimate_profitability(&p, &new_agent(), fees.total + 1000).unwrap();
        assert!(profitable);

        // Not profitable when profit < fees
        let (profitable, _) = calc.estimate_profitability(&p, &new_agent(), fees.total.saturating_sub(1)).unwrap();
        assert!(!profitable);
    }
}
