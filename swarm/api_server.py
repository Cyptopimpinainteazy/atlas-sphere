...
class SwarmAPIServer:
                ...
                while True:
                    try:
                        ...
                        try:
                            ...
                            logger.warning(f'Scheduled retrain failed: {e}')
                    except Exception as e:
    ...
    def setup_routes(self, app: web.Application):
        ...
        # Swarm health endpoints (for dashboard)
    ...
    async def get_swarm_health(self, request: web.Request) -> web.Response:
        """Get swarm health status - main dashboard endpoint"""
    ...
    async def get_swarm_agents(self, request: web.Request) -> web.Response:
        """Get list of swarm agents for dashboard"""
...
# Allow overriding via request params for fidelity experiments
    ...
    async def quantum_parameter_sweep(self, request: web.Request) -> web.Response:
        ...
        # Helper to run a single benchmark call and return (improvement_pct, quantum_won, duration_ms)
        def _run_benchmark_once(payload):
            try:
                import time
                start = time.perf_counter()
                resp = session.post(local_url, json=payload, timeout=timeout_per_call)
        ...
        # Execute sweep concurrently per-config
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as exe:
            ...
            for p in p_layers_list:
                for shots in shots_list:
                    ...
                    # submit all runs for this config
                    future_runs = [ exe.submit(_run_benchmark_once, pl) for pl in payloads ]
                    futures.append((p, shots, future_runs))

            # Collect results as they finish
            for p, shots, future_runs in futures:
                aggr_improvements = []
                aggr_wins = 0
                times = []
                for fut in future_runs:
                    try:
                        imp, won, dur = fut.result()
                        aggr_improvements.append(imp)
                        aggr_wins += 1 if won else 0
                        times.append(dur)
                    except Exception as e:
                        logger.warning(f'Error collecting sweep future: {e}')
    ...
    async def quantum_parameter_sweep_start(self, request: web.Request) -> web.Response:
        ...
        self._parameter_sweep_jobs[job_id] = job
        # Persist the job immediately
        try:
            import json, os
            path = os.path.join(self._jobs_dir, f"{job_id}.json")
            with open(path, 'w') as jf:
                json.dump(job, jf, indent=2)
            # update aggregate jobs index
        ...
        def _run_job():
            try:
                job['status'] = 'running'
                # Reuse synchronous sweep logic but collect into job['result']
                # Use default values similar to quantum_parameter_sweep
                size = int(data.get('size', 8))
                difficulty = data.get('difficulty', 'hard')
                p_layers_list = data.get('p_layers', [1,2,3])
                shots_list = data.get('shots', [512,1024,2048])
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as exe:
                    ...
                    for p in p_layers_list:
                        for shots in shots_list:
                            payloads = [ {'size': size, 'difficulty': difficulty, 'p_layers': int(p), 'shots': int(shots), 'refine_iters': int(data.get('refine_iters', 0))} for _ in range(runs_per) ]
                            future_runs = [ exe.submit(lambda pl=pl: (lambda: (session.post(local_url, json=pl, timeout=timeout_per_call).json() if session.post(local_url, json=pl, timeout=timeout_per_call).ok else None))()) for pl in payloads ]
                            futures.append((p, shots, future_runs))
                    ...
                    for p, shots, future_runs in futures:
                        ...
                        for fut in future_runs:
                            try:
                                ...
                                if not j:
                                    continue
                                aggr_improvements.append(j.get('improvement_pct') or 0.0)
                                if j.get('quantum_won'):
                                    aggr_wins += 1
                                # best-effort time
                                times.append(j.get('quantum_time_ms') or 0.0)
                            except Exception as e:
                        ...
                        win_rate = aggr_wins / runs_per
                        avg_time_ms = sum(times) / len(times) if times else float(timeout_per_call * 1000.0)
                        total_estimated_ms += avg_time_ms * runs_per
                        results.append({'p_layers': int(p), 'shots': int(shots), 'avg_improvement_pct': avg_imp, 'win_rate': win_rate, 'avg_time_ms': avg_time_ms})

                estimated_total_seconds = (total_estimated_ms / 1000.0) / max_workers if max_workers > 0 else (total_estimated_ms / 1000.0)
                best_by_imp = max(results, key=lambda x: x['avg_improvement_pct']) if results else None
                best_by_win = max(results, key=lambda x: x['win_rate']) if results else None

                job['result'] = {'results': results, 'best_by_improvement': best_by_imp, 'best_by_win': best_by_win, 'estimated_total_seconds': estimated_total_seconds}
                job['status'] = 'finished'
                # persist job result to disk
                try:
                    import json, os
                    path = os.path.join(self._jobs_dir, f"{job_id}.json")
                    with open(path, 'w') as jf:
    ...
    async def quantum_parameter_sweep_list(self, request: web.Request) -> web.Response:
        ...
        jobs_summary = []
        for jid, job in self._parameter_sweep_jobs.items():
            jobs_summary.append({
                'id': job['id'],
                'status': job.get('status'),
                'created': job.get('created'),
                'params': job.get('params'),
                'result': {'best_by_improvement': job.get('result', {}).get('best_by_improvement') if job.get('result') else None}
            })
