...
PY
  ...
  echo "Verifying tracked checksums from $(basename "$checksums")..."
  (
    cd "$ROOT"
...
$BINARY_SHA  $RELEASE_DIR_REL/bin/$BINARY_NAME
EOF
if [ -n "$WASM_SHA" ]; then
  cat >> "$CHECKSUM_FILE" <<EOF
$WASM_SHA  $RELEASE_DIR_REL/runtime/$(basename "$WASM_PATH")
EOF
fi
cat >> "$CHECKSUM_FILE" <<EOF
