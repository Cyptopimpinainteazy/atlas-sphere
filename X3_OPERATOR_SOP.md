...
- ✓ **Port Availability**: 9944 (RPC), 9945 (WS), 30333 (P2P), 9615 (Prometheus)
...
# Verify RPC/WS ports are truly free
...
3. **10-20 seconds**: RPC servers start (9944, 9945, 9615 become responsive)
...
- Port binding errors (9944, 9945 still in use)
...
ls -la ~/.x3/keys/
# Should have aura.json, grandpa.json, etc.

# Verify NODE_NAME matches key file
cat ~/.x3/keys/validator-name  # matches NODE_NAME
```
...
# Step 2: Start prod node (NO auto-kill, requires explicit --keep-ports)
...
cp -r /tmp/x3-dev /tmp/x3-dev.bad.$(date +%s)
