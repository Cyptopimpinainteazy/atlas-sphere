...
impl MockServiceManager {
    ...
    fn setup_external_chains() -> HashMap<String, MockExternalChain> {
        ...
        chains.insert("avalanche".to_string(), MockExternalChain {
            chain_id: 43114,
        ...
        chains.insert("bsc".to_string(), MockExternalChain {
            chain_id: 56,
        ...
        chains.insert("arbitrum".to_string(), MockExternalChain {
            chain_id: 42161,
        ...
        chains.insert("base".to_string(), MockExternalChain {
            chain_id: 8453,
            name: "Base".to_string(),
    ...
    async fn start_mock_dns_server(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let dns_routes = warp::path!("resolve" / String)
            .and(warp::get())
        ...
        let (_, server) = warp::serve(dns_routes)
            .bind_ephemeral(([127, 0, 0, 1], 5353));
    ...
    async fn start_mock_gpu_swarm(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        node_map.insert("gpu-node-1".to_string(), MockGPUNode {
        ...
        node_map.insert("gpu-node-2".to_string(), MockGPUNode {
        ...
        let (_, server) = warp::serve(swarm_routes)
    ...
    async fn start_mock_price_oracle(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Start price API
        let price_routes = warp::path!("price" / String)
            .and(warp::get())
        ...
        let (_, server) = warp::serve(price_routes)
            .bind_ephemeral(([127, 0, 0, 1], 8090));
        ...
        self.running_services.lock().unwrap().push(price_update_handle);
    ...
    async fn start_mock_block_explorer(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        tx_map.insert("0xtx123".to_string(), MockTransaction {
        ...
        let (_, server) = warp::serve(explorer_routes)
    ...
    async fn start_external_chain_mocks(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (name, chain) in &self.external_chains {
            ...
            let chain_rpc_routes = warp::path!("rpc")
                .and(warp::post())
                .and_then(move || {
                    async move {
                        Ok(reply::json(&serde_json::json!({
            ...
            let (_, server) = warp::serve(chain_rpc_routes)
                .bind_ephemeral(([127, 0, 0, 1], chain.port));
