...
impl TestContractManager {
    ...
    async fn deploy_lending_protocol(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pool_address = self.deploy_contract(
            "Pool",
        ...
        let configurator_address = self.deploy_contract(
            "PoolConfigurator",
            ContractType::LendingProtocol,
        ...
        let collateral_address = self.deploy_contract(
            "CollateralManager",
            ContractType::LendingProtocol,
        ...
        let interest_rate_address = self.deploy_contract(
            "InterestRateModel",
            ContractType::LendingProtocol,
    ...
    async fn deploy_ai_swarm_coordinator(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let address = self.deploy_contract(
            "AISwarmCoordinator",
    ...
    async fn deploy_cross_chain_position_manager(&mut self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn deploy_token_contract(&mut self, name: &str, initial_supply: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn deploy_contract(
        ...
        // Deploy via RPC
        let address = self.submit_deployment_transaction(name, &compiled_bytecode).await?;
    ...
    async fn compile_contract(&self, source_code: &str, bytecode: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    async fn submit_deployment_transaction(&self, name: &str, bytecode: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        if !response.status().is_success() {
            return Err(format!("Failed to deploy contract {}: {}", name, response.status()).into());
        }
