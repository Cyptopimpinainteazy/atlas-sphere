...
class CoordinatorHealthCheck(DeploymentCheck):
    ...
    def run(self) -> Tuple[CheckStatus, str]:
        try:
            ...
            return CheckStatus.FAIL, f"Exception: {str(e)}"
...
class ElasticsearchHealthCheck(DeploymentCheck):
    ...
    def run(self) -> Tuple[CheckStatus, str]:
        try:
            ...
            return CheckStatus.SKIP, f"Unavailable"
...
class CoordinatorPodsCheck(DeploymentCheck):
    ...
    def run(self) -> Tuple[CheckStatus, str]:
        try:
            ...
            return CheckStatus.SKIP, f"Cannot check: {str(e)}"
    ...
    def run_all(self) -> bool:
        ...
        for check in self.checks:
            try:
                ...
                logger.error(f"  ✗ EXCEPTION  {check.name:40} {str(e)}")
                total_fails += 1
                if check.critical:
                    critical_fails += 1
