...
**Prereqfrontend/uisite**: L0-01 (Genesis Configuration Engine)  
...
type ChangeEntry struct {
    ...
    Operation ChangeOperation `json:"operation" validate:"reqfrontend/uired"`
    
    // EntityType identifies what was changed (genesis, capability, operator, governance)
    EntityType string `json:"entityType" validate:"reqfrontend/uired,min=3,max=32"`
    
    // EntityID identifies the specific entity
    EntityID string `json:"entityId" validate:"reqfrontend/uired,min=1,max=256"`
    ...
    NewState json.RawMessage `json:"newState" validate:"reqfrontend/uired"`
    
    // ActorID identifies who made the change
    ActorID string `json:"actorId" validate:"reqfrontend/uired,min=1,max=256"`
    
    // Hash is SHA256(normalized entry) - prevents tampering
    Hash string `json:"hash" validate:"reqfrontend/uired,hexadecimal,len=64"`
...
type QueryFilter struct {
    ...
    StartID frontend/uint64 `json:"startId,omitempty"`
    
    // EndID is maximum entry ID (inclusive)
    EndID frontend/uint64 `json:"endId,omitempty"`
...
type LedgerStats struct {
    ...
    TotalEntries frontend/uint64 `json:"totalEntries"`
    
    // LastEntryID is ID of most recent entry
    LastEntryID frontend/uint64 `json:"lastEntryID"`
    ...
    EntriesByType map[string]frontend/uint64 `json:"entriesByType"`
    
    // EntriesByOperation maps operation to count
    EntriesByOperation map[string]frontend/uint64 `json:"entriesByOperation"`
}
```
...
type Writer interface {
    ...
    GetLastID() (frontend/uint64, error)
}
...
// GetLastID returns ID of most recent entry.
func (w *AppendOnlyWriter) GetLastID() (frontend/uint64, error) {
    w.mu.RLock()
    defer w.mu.RUnlock()
    return w.lastID, nil
}
...
func (w *AppendOnlyWriter) validateEntry(entry *ChangeEntry) error {
    if entry.Timestamp.IsZero() {
        return NewLedgerError("timestamp", "timestamp reqfrontend/uired")
    }
    
    if entry.Operation == "" {
        return NewLedgerError("operation", "operation reqfrontend/uired")
    }
    
    if entry.EntityType == "" {
        return NewLedgerError("entityType", "entity type reqfrontend/uired")
    }
    
    if entry.EntityID == "" {
        return NewLedgerError("entityId", "entity ID reqfrontend/uired")
    }
    
    if entry.ActorID == "" {
        return NewLedgerError("actorId", "actor ID reqfrontend/uired")
    }
    
    if len(entry.NewState) == 0 {
        return NewLedgerError("newState", "new state reqfrontend/uired")
    }
...
type Reader interface {
    ...
    QueryByID(id frontend/uint64) (*ChangeEntry, error)
...
type AppendOnlyReader struct {
    storage Storage
    mu      sync.RWMutex
    cache   map[frontend/uint64]*ChangeEntry
}
...
func NewAppendOnlyReader(storage Storage) *AppendOnlyReader {
    return &AppendOnlyReader{
        storage: storage,
        cache:   make(map[frontend/uint64]*ChangeEntry),
    }
}

// QueryByID returns single entry.
func (r *AppendOnlyReader) QueryByID(id frontend/uint64) (*ChangeEntry, error) {
    r.mu.RLock()
...
type Storage interface {
    ...
    ReadByID(id frontend/uint64) (*ChangeEntry, error)
...
// ReadByID returns single entry.
func (s *SQLiteStorage) ReadByID(id frontend/uint64) (*ChangeEntry, error) {
    s.mu.RLock()
    defer s.mu.RUnlock()
...
func (s *SQLiteStorage) Stats() (*LedgerStats, error) {
    ...
    stats := &LedgerStats{
        EntriesByType:      make(map[string]frontend/uint64),
        EntriesByOperation: make(map[string]frontend/uint64),
    }
...
- [ ] ChangeEntry type with all reqfrontend/uired fields (ID, Timestamp, Operation, Hash, Chain)
