...
embedded directly inside the Atlas node. Validators expose sandboxed GPU workers
...
- Atlas validators can offer **zero-marginal-cost compute** since the
...
| `pallets/atlas-kernel/` | Couple with `pallet-depin-marketplace` for fee accounting |
...
- [ ] How does this interact with the existing `GPUMarketplace.sol` contract?
      (Propose: bridge via cross-VM comit in atlas-kernel; eventually migrate to native pallet.)
