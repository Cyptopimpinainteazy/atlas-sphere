...
state thrashing — making Atlas the first L1 with **adaptive parallelism**
instead of static.
...
The current Atlas pipeline (SigVerifier → PoH → GPU batch execution)
...
| `pallets/atlas-kernel/` | Emit access traces for training data collection |
